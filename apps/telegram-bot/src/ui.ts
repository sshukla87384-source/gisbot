...
export function mainMenuKeyboard(user: BotUser, cfg: Record<string, { label?: string; icon?: string } | undefined> = {}): InlineKeyboard {
  ...
  const kb = new InlineKeyboard()
    ...
    .row()
    .add(sbtn(L("search", t(loc, "b_search")), cb("shp", "find"), "primary", I("search")), sbtn(L("licenses", t(loc, "b_licenses")), cb("lic", "list", 1), "primary", I("licenses")))
    .row()
    .add(sbtn("🔄 Replacement", cb("rep", "home"), "primary"), sbtn(L("support", t(loc, "b_helpsupport")), cb("sup", "home"), "primary", I("support")))
    .row()
